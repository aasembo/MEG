    ...
    <div class="col-md-3">
        <div class="card border-0 shadow-sm bg-info text-white">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h6 class="card-subtitle mb-2 text-white-50">Roles</h6>
                        <h3 class="mb-0"><?php echo count($roleCounts) ?></h3>
                    </div>
                    <div class="fs-1 opacity-50">
        ...
        <?php echo $this->Form->create(null, ['type' => 'get', 'class' => 'row g-3']) ?>
            ...
            <div class="col-md-2">
                <?php echo $this->Form->control('hospital_id', [
                    'type' => 'select',
                    'options' => ['' => 'All Hospitals', '0' => 'No Hospital (Super Users)'] + $hospitals,
                    'class' => 'form-select',
                    'label' => false,
                    'value' => $this->request->getQuery('hospital_id'),
                    'div' => false,
                    'templates' => [
                        'inputContainer' => '{{content}}',
                        'select' => '<select name="{{name}}" {{attrs}}>{{content}}</select>'
...
<div class="row mb-4">
    ...
    <div class="col-md-4">
        <?php echo $this->Html->link('', ['action' => 'index', '?' => ['role_id' => $roleCount->role_id ?? '']], ['class' => 'text-decoration-none']) ?>
        <div class="card border-0 shadow-sm">
            <div class="card-body text-center">
                <div class="mb-2">
                    <span class="badge bg-<?php echo $roleCount->role_type === 'super' ? 'warning' : ($roleCount->role_type === 'admin' ? 'info' : 'secondary') ?> fs-6">
                        <?php echo h($roleCount->role_name) ?>
                    </span>
                </div>
                <h4 class="mb-0"><?php echo $roleCount->count ?></h4>
    ...
    <div class="card-body p-0">
                ...
                <table class="table table-hover mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th scope="col"><?php echo $this->Paginator->sort('hospital_id', 'Hospital') ?></th>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($user->hospital): ?>
                                    <?php echo $this->Html->link(
                                        '<span class="badge bg-light text-dark border">' . h($user->hospital->name) . '</span>',
                                        ['controller' => 'Hospitals', 'action' => 'view', $user->hospital->id],
                                        ['escape' => false, 'title' => 'View hospital details']
                                    ) ?>
                                <?php else: ?>
        ...
        <?php else: ?>
            <div class="text-center py-5">
                ...
                <p class="text-muted">
                    <?php if ($this->request->getQuery('search') || $this->request->getQuery('status') || $this->request->getQuery('role_id') || $this->request->getQuery('hospital_id')): ?>
