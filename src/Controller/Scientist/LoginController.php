...
/**
 * Scientist Login Controller
 *
 * Handles authentication for scientists using OpenID Connect
 */
class LoginController extends AppController
{
    ...
    /**
     * Scientist login action - redirects to Okta
     *
    ...
    public function login()
    {
        ...
        // For scientist access, we need a valid hospital context
        if (!$hospital) {
            $this->Flash->error(__('Scientist access requires a valid hospital context. Please access from a hospital subdomain or the main site for Hospital1 access.'));
        ...
        if ($result && $result->isValid()) {
            ...
            if ($user) {
                ...
                if ($userWithRole && $userWithRole->role) {
                    // Check if user has scientist role type only
                    if ($userWithRole->role->type === 'scientist') {
        ...
        if ($this->request->is('post')) {
            ...
            if ($result && $result->isValid()) {
                ...
                if ($userWithRole && $userWithRole->role) {
                    // Check if user has scientist role type
                    if ($userWithRole->role->type === 'scientist') {
                        // Check if user belongs to current hospital context
                        if ($userWithRole->hospital_id != $hospitalId) {
                            $this->Flash->error(__('You do not have access to this hospital\'s scientist panel.'));
                        ...
                        $this->activityLogger->logLogin($userWithRole->id, [
                            ...
                            'description' => 'Scientist logged in via form authentication'
                        ]);
        ...
        if (\Cake\Core\Configure::read('Okta.enabled', false)) {
            ...
            if ($this->request->is('get')) {
                $oktaLoginUrl = $this->buildOktaLoginUrl('scientist');
    ...
    /**
     * Scientist logout action - logs out from local session and Okta if applicable
     *
        ...
        if ($result && $result->isValid()) {
            ...
            if ($loginMethod === 'okta') {
                $this->activityLogger->logOktaLogout($userId, [
                    'role_type' => 'scientist',
                    'request' => $this->request,
                    'description' => 'Scientist logged out from Okta',
                    'event_data' => [
                ...
                $this->activityLogger->logLogin($userId, [
                    'role_type' => 'scientist',
                    'request' => $this->request,
                    'description' => 'Scientist logged out from form authentication',
                    'event_data' => [
    ...
    private function buildOktaLoginUrl(string $role): ?string
    {
        ...
        $this->log("Scientist login - Hospital ID: " . ($hospitalId ?: 'NONE') . ", Hospital Subdomain: " . ($hospitalSubdomain ?: 'NONE') . ", Hospital Name: " . ($hospital ? $hospital->name : 'NONE'), 'info');
        ...
        if (!$hospitalId && !$hospitalSubdomain) {
            ...
            $this->log("Scientist login - No hospital context provided, using default subdomain: hospital1", 'info');
            ...
            if ($defaultHospital) {
                ...
                $this->log("Scientist login - Set default hospital context: {$defaultHospital->name} (ID: {$hospitalId})", 'info');
            } else {
