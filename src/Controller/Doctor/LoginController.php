 ...
 *
 * Handles authentication for doctors using OpenID Connect
 */
class LoginController extends AppController
{
    ...
    /**
     * Doctor login action - redirects to Okta
     *
    ...
    public function login()
    {
        ...
        // For doctor access, we need a valid hospital context
        if (!$hospital) {
            $this->Flash->error(__('Doctor access requires a valid hospital context. Please access from a hospital subdomain or the main site for Hospital1 access.'));
        ...
        if ($result && $result->isValid()) {
            ...
            if ($user) {
                ...
                if ($userWithRole && $userWithRole->role) {
                    // Check if user has doctor role type only
        ...
        if ($this->request->is('post')) {
            ...
            if ($result && $result->isValid()) {
                ...
                if ($userWithRole && $userWithRole->role) {
                    // Check if user has doctor role type
                    if ($userWithRole->role->type === 'doctor') {
                        // Check if user belongs to current hospital context
                        if ($userWithRole->hospital_id != $hospitalId) {
                            $this->Flash->error(__('You do not have access to this hospital\'s doctor panel.'));
                        ...
                        $this->activityLogger->logLogin($userWithRole->id, [
                            ...
                            'description' => 'Doctor logged in via form authentication'
                        ]);
                        
                        $this->Flash->success(__('Welcome, Dr. {0}!', $userWithRole->first_name));
                        return $this->redirect([
                            'prefix' => 'Doctor',
                            'controller' => 'Dashboard', 
                            'action' => 'index'
        ...
        if (\Cake\Core\Configure::read('Okta.enabled', false)) {
            ...
            if ($this->request->is('get')) {
                $oktaLoginUrl = $this->buildOktaLoginUrl('doctor');
    ...
    /**
     * Doctor logout action - logs out from local session and Okta if applicable
     *
        ...
        if ($result && $result->isValid()) {
            ...
            if ($loginMethod === 'okta') {
                $this->activityLogger->logOktaLogout($userId, [
                    'role_type' => 'doctor',
                    'request' => $this->request,
                    'description' => 'Doctor logged out from Okta',
                    'event_data' => [
                ...
                $this->activityLogger->logLogin($userId, [
                    'role_type' => 'doctor',
                    'request' => $this->request,
                    'description' => 'Doctor logged out from form authentication',
                    'event_data' => [
    ...
    private function buildOktaLoginUrl(string $role): ?string
    {
        ...
        if (!$hospitalId) {
            $this->Flash->error(__('Hospital context is required for doctor access. Please access from a valid hospital subdomain.'));
            return null;
        }
